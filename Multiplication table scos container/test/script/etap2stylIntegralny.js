...
$(document).ready( function() {
	...
	var objective = getObjectiveStatus("step2style3");
		...
		$('#zad2input1').keyup(function(){
			if ($('#zad2input2').val()) {
				...
				if (firstNumber==5 && secondNumber==7) {
					...
					//if byl juz w czynnosciowy to koniec
					var nextObjective = getObjectiveStatus('step2style1');
					if (nextObjective=='failed'){
						setObjectiveStatus('courseFailed', 0);
						$('#errorMessage3Final').removeClass('is-hidden');
						setObjectiveStatus('step2style3', 0);
					}
					else {
						setObjectiveStatus('courseFailed', 1);
